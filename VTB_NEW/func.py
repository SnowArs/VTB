...
def culc(ticker, error_array, prof_per_year_dict, prof_rur_per_year_dict):
                        ...
                        if bought_amount_left < 0:
                            ...
                            ticker, prof_per_year_dict, prof_rur_per_year_dict \
                                = profit_calculation(ticker, 1, sale_row_number, sold_volume_in_loop,
                                                     prof_per_year_dict, prof_rur_per_year_dict, i)
    ...
    return ticker, error_array, prof_per_year_dict, prof_rur_per_year_dict
...
def profit_calculation(ticker, option, sale_row_number, sold_volume, prof_per_year_dict, profit_rur_per_year_dict, i=0):
    ...
    if option == 0:  # разница бумаг 0 или положительная
        # commission = ticker.commission * ticker.df['RUB_sum'][sale_row_number]
        ...
        prof_usd = sold_volume * ticker.df['Price'][sale_row_number] - sold_volume * \
                   ticker.df['Price'][buy_row_number]
    ...
    ticker.prof_for_sold_securities_rur += prof_rur
    ...
    if ticker.df.shape[0] == row_to_fill:
        new_row = {'profit_rus': prof_rur, 'prof_usd': prof_usd}
    ...
    profit_rur_per_year_dict[str(ticker.df.iloc[:, sets['Дата']][sale_row_number].year)].append(round(prof_rur, 2))
    return ticker, prof_per_year_dict, profit_rur_per_year_dict
...
def rub_securities_processing(ticker, error_array):
    ...
    if ticker.outstanding_volumes == 0:
        ...
        ticker.prof_for_sold_securities = prof_for_sold_securities
        ...
        ticker.profit_for_outstanding_volumes = round(profit_for_outstanding_volumes, 2)
    else:
        prof_for_sold_securities = ticker.sell_sum_for_rub_securities - ticker.total_sell * ticker.average_buy
        ticker.prof_for_sold_securities = round(prof_for_sold_securities, 2)
        profit_for_outstanding_volumes = (ticker.current_price - ticker.average_buy) * ticker.outstanding_volumes
