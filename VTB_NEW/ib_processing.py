...
# full_list_of_securities = ['USD.HKD']
df = filling_roe(df, 0, 3)
broker = 'IB'
errors = []
df2020 = pd.read_excel('calc\\ib2020.xls')
df = df2020.append(df,ignore_index=True, sort=True)
df = df.drop(df.loc[df['B/S'] ==''].index)
df.reset_index(drop=True, inplace=True)
df = df[['date', 'Символ', 'B/S', 'Валюта', 'Price', 'Volume', 'Comisson', 'Sum', 'ROE_index', 'ROE', 'RUB_sum']]
...
errors_df.to_excel('calc/ERRORS.xls')
print()
print(mytable_rus)
